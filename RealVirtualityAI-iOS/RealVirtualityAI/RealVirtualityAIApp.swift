    ...
    var body: some Scene {
        WindowGroup {
            RootView()
                ...
                .tint(tema.c1)
                // GÜVENLİK: önce App Attest token'ı hazırla (race önle), SONRA veri çek (ENFORCE'ta ilk istek token'sız gitmesin)
                .task { await AppAttest.shared.hazirla(); await api.durumYukle() }
